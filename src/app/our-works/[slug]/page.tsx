...
const WorkDetails = ({ params }: { params: { slug: string } }) => {
  ...
  const overviewCards = [
    { title: "Challenge", content: details.challenge },
    { title: "Solution", content: details.solution },
    { title: "Result", content: details.result },
  ]
  ...
  const steps = ["Research", "Userflow", "Wireframes", "Design", "Development"]
  ...
  const stepVariant = {
    ...
    visible: (i: number) => ({ opacity: 1, x: 0, transition: { delay: i * 0.2, duration: 0.5 } })
  }
      ...
      <section className="relative max-w-full sm:max-w-[95%] md:max-w-[90%] mx-auto px-4 sm:px-6 md:px-4 py-12 sm:py-14 md:py-16 mt-8 sm:mt-10 md:mt-12 lg:mt-18">
        ...
        <header>
          <div className="space-y-3 sm:space-y-4 mb-8 sm:mb-10 text-left w-full sm:w-[80%] md:w-[60%] lg:w-[45%]">
            <h1 className="text-[28px] sm:text-[36px] md:text-[48px] lg:text-[64px] font-semibold md:font-medium text-[#121212] leading-tight">{title}</h1>
        ...
        {/* COVER IMAGE */}
        <motion.div
          className="rounded-2xl sm:rounded-3xl overflow-hidden mb-10 sm:mb-12"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
        ...
        {/* OVERVIEW CARDS */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 mt-12 sm:mt-16 md:mt-20 lg:mt-24">
          {overviewCards.map((item, index) => {
            const isLast = index === overviewCards.length - 1
            return (
              <motion.div
                key={item.title}
                className={`rounded-2xl p-6 sm:p-7 md:p-[32px] flex flex-col justify-between transition-colors duration-300 w-full min-h-[350px] sm:min-h-[400px] md:min-h-[440px] lg:min-h-[476px] ${isLast ? "bg-[#544BC2] text-white" : "bg-[#F7F7F7] text-[#121212]"}`}
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true }}
              ...
              >
                <div className="flex flex-col justify-between h-full">
                  <h3 className={`text-[20px] sm:text-[22px] md:text-[24px] font-medium mb-4 sm:mb-5 md:mb-6 ${isLast ? "text-[#FFFFFFCC]" : "text-[#12121299]"}`}>{item.title}</h3>
                  <p className={`text-[16px] sm:text-[18px] md:text-[18px] leading-relaxed ${isLast ? "text-white" : "text-[#121212]"}`}>{item.content}</p>
                </div>
              </motion.div>
            )
          })}
        ...
        {/* DESKTOP SCREENS */}
        <motion.div
          className="my-16 sm:my-20 md:my-24"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
          variants={imageVariant}
          ...
          <div className="relative w-full bg-[#DEDBDB] rounded-[2.14px] p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 md:gap-8 lg:gap-10 overflow-x-auto">
              {desktopImagesArray.map((img, imgIndex) => (
                <motion.div key={imgIndex} className={`flex-shrink-0 ${desktopImagesArray.length === 1 ? 'w-full' : 'w-full sm:w-[calc(50%-12px)] md:w-[calc(33.333%-16px)] lg:w-[calc(25%-24px)]'}`} variants={imageVariant}>
                  <Image
                    src={img}
          ...
          <div className="relative flex flex-col items-start mt-16 sm:mt-20 md:mt-24">
            ...
            {steps.map((step, index) => (
              <motion.div
                ...
                style={{ left: `calc(${index} * var(--step-x))`, top: `calc(${index} * var(--step-y))` }}
                custom={index}
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true }}
              ...
              >
                <span className="text-[#544BC2] font-medium text-[14px] sm:text-base md:text-[20px] lg:text-[26px]">{step}</span>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        {/* TESTIMONIAL */}
        <motion.div className="max-w-full sm:max-w-4xl md:max-w-5xl mx-auto space-y-4 sm:space-y-6 md:flex md:items-center md:justify-between md:gap-12 lg:gap-20" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={testimonialVariant}>
          <div className={`${project.slug === "perfection-reloaded" ? "bg-black" : "bg-transparent"} md:w-[1800px] h-full rounded-lg flex items-center justify-center overflow-hidden shadow-[#544BC224] p-6`}>
            <Image src={details.testimonial.image} alt={details.testimonial.name} width={403} height={547} className="object-contain max-w-[343px] sm:max-w-[380px] md:max-w-[340px] h-[400px] sm:h-[450px] md:h-[480px] lg:h-[500px] rounded-lg" quality={100} sizes="(max-width: 768px) 90vw, 403px" />
          </div>

          <div className="space-y-2 sm:space-y-3">
            <h4 className="font-semibold text-[#121212] text-[20px] sm:text-[22px] md:text-[24px]">{details.testimonial.name}</h4>
            <small className="text-[14px] sm:text-[15px] md:text-[16px] text-[#12121299] font-medium block">{details.testimonial.role}</small>
            <p className="text-[#12121280] text-[16px] sm:text-[20px] md:text-[26px] lg:text-[32px] leading-relaxed">&ldquo;{details.testimonial.feedback}&ldquo;</p>
          </div>
        </motion.div>
      </section>
